...
👉 **[新手部署教程（零基础版）](./新手部署教程.md)** —— 从「怎么装 Python」开始，
每一步都写清楚「在哪里、输入什么、会看到什么」，含 9 条常见问题排查。

👉 **[在 PyCharm 里使用](./PyCharm使用指南.md)** —— 打开哪个目录、配解释器、运行、调试、
跑测试、怎么配密钥，以及 PyCharm 特有的坑。

**Windows 上最简单的方式**：双击 `启动.bat`（启动）或 `自检.bat`（体检），不用敲任何命令。
