...
### ➡️ [`academic-galgame-windows.zip`](./download/academic-galgame-windows.zip) · 10.8 MB

| 步骤 | 做什么 |
|---|---|
| 1 | 下载后**解压**（右键 → 全部解压缩） |
| 2 | 双击里面的 **`学术galgame.exe`** |
| 3 | 浏览器自动打开 → 点右上角 **「⚙ 设置」** 填上你自己的大模型 Key → 开始玩 |

- ✅ **不用装 Python，也不用装 Node** —— 解释器连同游戏代码已经打包在包里了
- ✅ 第一次运行会自动在桌面建一个带鲸鱼娘立绘图标的快捷方式
- ✅ 纯绿色版：删掉文件夹就卸载干净，不写注册表
- 🔧 出问题：双击 `自检.bat`（打印诊断报告）或 `排查模式.bat`（看运行日志）
- 📖 包内附 `使用说明.txt`，从「第一次填什么」到「怎么卸载」都写了

> 这个桌面版打包的是 [Python 版](#-python-版纯标准库)（同一套引擎与前端）。
> 想自己从源码构建，见 [`desktop/README.md`](./desktop/README.md)。
...
- [它是什么](#它是什么)
- [下载即用（Windows 桌面版）](#下载即用windows-桌面版)
- [快速开始](#快速开始)
...
内置的 5 个学科只是**开箱即用的示例**。你可以：

- **从知识库一键添加**：② 拉取列表后，某个知识库右侧点 **「+ 添加为学科」** → 学科立刻出现在游戏里，**并自动绑定该知识库**。
- **手动添加**：③ 里输入学科名 → 「+ 添加学科」；主面板「学科熟练度」标题旁的 **＋** 也能快速加。
- **逐个绑定/解绑知识库**：③ 每个学科右侧的下拉框，可换绑别的知识库或设为「未绑定」。
- **删除学科**：③ 里点 `×`（会连带删除该学科的熟练度与任务链；**至少保留一个**）。

所以每个人的「课程表」就是他**自己知识库的镜像** —— 你 ima 里有什么，游戏里就能学什么。

> 老师（LLM）只会教当前学科列表里的科目，不会自己臆造学科；你想学新科目时，它会提示你去添加。

### ③ 我的教材：能做什么

- **拖拽上传**：把文件拖进虚线框即可，也可点击选择；**可分多次追加**，同名文件自动覆盖。
- **支持格式**：`.md` / `.txt` / `.docx`（可靠）；`.pdf` **实验性**（简单 PDF 可用，扫描件/特殊字体 PDF 会失败并给出提示）。
  - 文本在**你的浏览器里**提取（`.docx` 解 ZIP 取 `word/document.xml`；`.pdf` 尽力解压内容流），**零依赖、零上传**。
- **多文件合并**：所有文件合并成一个可检索语料库，按 Markdown 标题切小节；列表显示每个文件的字数与合计。
- **按学科指定**：每个文件右侧下拉框 ——
  - **通用（所有学科）**：任何学科都能检索到（默认）
  - **具体学科**：这份教材**只**在该学科下被检索，教别的科目时不会串味
- **单文件删除**：点右侧 `×` 移除某一个；「清空我的教材」移除全部。
- **刷新不丢**：教材同时镜像到**你浏览器的 IndexedDB**，刷新或重开浏览器会自动恢复到当前会话，不用重新上传。
...
> ⚠️ **配了 ima 就不会「悄悄」用内置示例**：以前 ② 没命中会静默回落到 ③，
> 于是你以为老师在讲你的书，其实讲的是仓库里的两篇示例。现在回落时会带上明确的
> `note` 说明「这不是你的资料」，让老师如实告诉用户。
...
```powershell
# Windows
cd workbuddy; .\install.ps1
```
```bash
# macOS / Linux
cd workbuddy && chmod +x install.sh && ./install.sh
```
...
│       ├── index.html
│       ├── styles.css
│       ├── app.js                #   界面逻辑 + BYOK 凭证管理
│       ├── extract.js            #   教材文本提取（.md/.txt/.docx/.pdf）
│       ├── store.js              #   教材本地持久化（IndexedDB）
