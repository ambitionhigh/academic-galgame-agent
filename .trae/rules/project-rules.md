...
- **模型后端**：统一走**火山引擎 · 火山方舟（Ark）**的 OpenAI 兼容接口，经 `src/agent/ark.js` 调用。禁止把 API Key 写进代码。
...
| `src/agent/` | LLM 编排（人设、工具、检索） | 只依赖 `engine/`；所有模型调用经 `ark.js` |
...
- 密钥只从环境变量读取（`ARK_API_KEY` 等），`.env` 已在 `.gitignore` 中。
